...
class MessageHistoryServiceTest {
  ...
  @BeforeEach
  void setUp() {
    messageHistoryService = new MessageHistoryService(repository);
  }

  private static MessageHistoryDocument message(
  ...
  void getHistoryMapsMessagesNewestFirst() {
    ...
    MessageHistoryDocument newer =
        message("alice_bob", "bob@example.com", "alice@example.com", "hi", t2);
    MessageHistoryDocument older =
        message("alice_bob", "alice@example.com", "bob@example.com", "yo", t1);
    ...
    List<MessageHistoryResponse> history =
        messageHistoryService.getHistory("alice@example.com", "bob@example.com", null, 50);
  ...
  void getHistoryWithBeforeCursorUsesTimestampFilter() {
    ...
    messageHistoryService.getHistory("alice@example.com", "bob@example.com", before, 20);
  ...
  void getConversationsGroupsByConversationAndKeepsLatest() {
    ...
    MessageHistoryDocument aliceToBob =
        message(
            ...
            "first",
            t1);
    MessageHistoryDocument bobToAlice =
        message(
            "alice@example.com_bob@example.com",
            "bob@example.com",
            "alice@example.com",
            "second",
            t2);
    MessageHistoryDocument carolToAlice =
        message(
    ...
    List<ConversationSummaryResponse> conversations =
        messageHistoryService.getConversations("alice@example.com");
