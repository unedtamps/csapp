package com.mycomp.csmessage.chats;
...
class ChatWebSocketIntegrationTest {
  ...
  private static final String VHOST = "cs-message";
  private static final String BROKER_USER = "cs-message";
  private static final String BROKER_PASS = "cs-message-pass";
  ...
  static void registerProperties(DynamicPropertyRegistry registry) {
    ...
    registry.add("spring.mongodb.uri", () -> mongo.getConnectionString() + "/cs_message");
  ...
  void privateMessageIsDeliveredToRecipientQueueAndPersisted() throws Exception {
    ...
    try {
      ...
      List<ChatMessageDto> recipientReceived = new CopyOnWriteArrayList<>();
      ...
      List<ChatMessageDto> senderReceived = new CopyOnWriteArrayList<>();
      ...
      ChatMessageDto delivered = recipientReceived.get(0);
      ...
      String conversationId = ConversationUtils.conversationId(senderEmail, recipientEmail);
      await()
          .atMost(10, TimeUnit.SECONDS)
          .pollInterval(200, TimeUnit.MILLISECONDS)
          .untilAsserted(
              () -> {
                ...
                MessageHistory latest = history.get(0);
  ...
  private StompSession connect(String email) throws Exception {
    ...
    StompHeaders headers = new StompHeaders();
    headers.set("Authorization", "Bearer " + jwtUtil.generateAccessToken(user(email)));
  ...
  private static class RecordingFrameHandler implements StompFrameHandler {
    ...
    private RecordingFrameHandler(List<ChatMessageDto> messages) {
